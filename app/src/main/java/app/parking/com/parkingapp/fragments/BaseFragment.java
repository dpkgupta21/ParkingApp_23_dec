package app.parking.com.parkingapp.fragments;
...
public class BaseFragment extends Fragment implements OnClickListener, OnCheckedChangeListener {
    ...
    /**
     * Method use to set tag on TextView, EditText, Button.
     *
     * @param id   resource of TextView, EditText, Button.
     * @param tag  string you want to set on TextView, EditText, Button
     * @param view
     */
    public void setViewTag(int id, String tag, View view) {
        View v = ((View) view.findViewById(id));
        if (v instanceof TextView) {
            TextView tv = (TextView) v;
            tv.setTag(tag);
    ...
    /**
     * Method use to get tag from TextView, EditText, Button.
     *
     * @param id   resource id TextView, EditText, Button
     * @param view
     * @return string text from view
     */
    public String getViewTag(int id, View view) {
        String text = "";
        View v = ((View) view.findViewById(id));
        if (v instanceof TextView) {
            TextView tv = (TextView) v;
            text = tv.getTag() == null ? "" : tv.getTag().toString().trim();
        }
        if (v instanceof EditText) {
            EditText et = (EditText) v;
            text = et.getTag() == null ? "" : et.getTag().toString().trim();
        }
        if (v instanceof Button) {
            Button btn = (Button) v;
            text = btn.getTag() == null ? "" : btn.getTag().toString().trim();
        }
        return text;
    }
